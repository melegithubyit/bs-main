...
export default function ApplicantList({ applicants, layout, onLayoutChange }: ApplicantListProps) {
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div>
                    ...
                    <p className="text-sm text-gray-500">Showing {sortedApplicants.length} results</p>
                </div>
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
                    ...
                    <Button className="bg-orange-500 hover:bg-orange-600 text-white w-full sm:w-auto">Apply for Fund</Button>
                </div>
            </div>
            ...
            {layout === "list" ? (
                <div className="space-y-4">
                    {currentApplicants.length > 0 ? (
                        currentApplicants.map((applicant, index) => (
                            <ApplicantCard key={applicant.id} applicant={applicant} index={index} layout="list" />
                        ))
                    ) : (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {currentApplicants.length > 0 ? (
                        currentApplicants.map((applicant, index) => (
                            <ApplicantCard key={applicant.id} applicant={applicant} index={index} layout="grid" />
                        ))
                    ) : (
