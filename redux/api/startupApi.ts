...
export const startupApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    updateStartupApproval: builder.mutation<any, { id: string; startup: StartupApproval }>({
        query: ({ id, startup }) => ({
    ...
    // update a startup
    updateStartup: builder.mutation<any, { id: string; startup: creatingStartupPayload }>({
        query: ({ id, startup }) => ({
            url: `/startup/${id}`,
            method: "PATCH",
            body: startup,
